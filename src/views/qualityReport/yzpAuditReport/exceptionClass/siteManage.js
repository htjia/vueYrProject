...
import { selectAll, adds, updates, enableAndDisable, deletes, updateSerialNumber } from '@/api/baobiao/exceptionClass'
...
export default {
  ...
  methods: {
    ...
    updateSequence() {
      ...
      updateSerialNumber(params).then(res => {
        this.$message({
          type: 'success',
          message: '调整成功'
        })
    ...
    setStatus(row) {
      if (row.isChecked) {
        this.$confirm('确定启用?', '提示', {
          ...
          enableAndDisable(requestParams).then(res => {
            this.$message({
              type: 'success',
              message: '修改成功!'
            })
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.list.length !== 0) {
            serialNumber = this.list[this.list.length - 1].serialNumber + 1
          }
          const params = {
