...
import { findMachineByProcess, findProgramByProcessId, xpSpcConfigSK, xpSpcConfigFind } from '@/api/chipManage/reportManage/spc'
...
export default {
  ...
  methods: {
    ...
    importExcel() {
      ...
      window.open(process.env.BASE_API + `/xpSpc/exportSK?machine=${this.machine}&programId=${this.process}&startTime=${startTime}&endTime=${endTime}`)
    ...
    fetchData() {
      ...
      xpSpcConfigSK(requestParams).then(res => {
        ...
        for (const item of this.anotherLists) {
          ...
          data1.push(item.skAvg)
          data2.push(item.skmr)
          if (max1 < parseFloat(item.skAvg)) {
            max1 = parseFloat(item.skAvg)
          }
          if (max2 < parseFloat(item.skmr)) {
            max2 = parseFloat(item.skmr)
          }
        }
