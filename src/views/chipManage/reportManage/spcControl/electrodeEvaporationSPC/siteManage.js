...
import { findMachineByProcess, findProgramByProcessId, xpSpcConfigDJ, xpSpcConfigFind } from '@/api/chipManage/reportManage/spc'
...
export default {
  ...
  methods: {
    ...
    importExcel() {
      ...
      window.open(process.env.BASE_API + `/xpSpc/exportDJ?machine=${this.machine}&programId=${this.process}&startTime=${startTime}&endTime=${endTime}`)
    ...
    fetchData() {
      ...
      xpSpcConfigDJ(requestParams).then(res => {
        ...
        for (const item of this.anotherLists) {
          x.push(item.run)
          data1.push(item.djhd)
          data2.push(item.djhdmr)
          if (max1 < parseFloat(item.djhd)) {
            max1 = parseFloat(item.djhd)
          }
          if (max2 < parseFloat(item.djhdmr)) {
            max2 = parseFloat(item.djhdmr)
          }
        }
