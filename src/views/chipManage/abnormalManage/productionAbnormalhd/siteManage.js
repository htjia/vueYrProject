...
import { findCreator, findAuditor, findModel, customerList, findAfter, determine, updateAfter, gyFlowCardFind, queryBack, exceptionBill, findErrorList, fetchCardInfo, completeDeterMine, getDetails } from '@/api/chipManage/abnormalManage/productionAbnormal'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        flowCardId: [{ required: true, message: '请选择返工工序', trigger: 'blur' }],
  ...
  methods: {
    ...
    // 查询
    customerList() {
      findCreator().then(res => {
        this.userLists = res.data
      })
      findAuditor().then(res => {
        this.auuserLists = res.data
      })
      customerList().then(res => {
        this.cuserLists = res.data.list
      })
    },
    // 返工单号
    ...
    fetchData() {
      ...
      const params = {
        ...
        reason: this.reason,
        model: this.model,
        waferNos: this.mulForm.mulWaferID,
        createTimeBefore: this.beginDate1 === '' ? '' : this.formatDate(this.beginDate1),
        createTimeAfter: this.endDate1 === '' ? '' : this.formatDate(this.endDate1),
        auditTimeBefore: this.beginDate === '' ? '' : this.formatDate(this.beginDate),
        auditTimeAfter: this.endDate === '' ? '' : this.formatDate(this.endDate)
      }
      findAfter(params).then(res => {
      ...
      if (this.radio === 1) {
        this.$refs['backForm'].validate((valid) => {
          if (valid) {
            ...
            params.processId = this.backForm.flowCardId
            params.responsible = this.backForm.creator.join()
            params.reason = this.backForm.reason
            params.prevent = this.backForm.prevent
            updateAfter(params).then(res => {
              if (res.code === 0) {
                this.$message({
                  type: 'success',
                  message: '提交成功!'
        ...
        this.$refs['backForms'].validate((valid) => {
          if (valid) {
            params.processId = this.backForms.flowCardId
            params.responsible = this.backForms.creator.join()
            params.applyDept = sessionStorage.getItem('deptId')
            params.responsibleDept = this.backForms.dept.join()
            params.reason = this.backForms.reason
            params.prevent = this.backForms.prevent
            updateAfter(params).then(res => {
              if (res.code === 0) {
                this.$message({
                  type: 'success',
    ...
    opennew() {
      ...
      const record = {
        ...
        batchNo: '',
        details: details,
        model: this.multipleSelection[0].model,
        process: this.multipleSelection[0].checkName
      }
