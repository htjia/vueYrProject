...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('glueCoatHeaders') === null ? this.formTheadOptions : localStorage.getItem('glueCoatHeaders').split(',')
  ...
  methods: {
    ...
    inputChange(data) {
      if (data.paramsCode === 'tubushijian' && parseInt(data.val) < 0) {
        data.val = 0
      }
    },
    getParamsData(row) {
    ...
    submitOption() {
      ...
      localStorage.setItem('glueCoatHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=7&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    // 扫描回车
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=7&siteName=光刻胶涂布&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    handleDeliverSubmit() {
      ...
      paramsList.jiaoguanhao = this.rubberNum
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 7 }).then(res => {
