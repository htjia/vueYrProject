...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('annealHeaders').split(',')
  ...
  methods: {
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=15&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    inputChange(data) {
      if (data.paramsCode === 'tuihuoshijian' && parseInt(data.val) < 0) {
        data.val = 0
      }
    },
    getParamsData(row) {
    ...
    submitOption() {
      ...
      this.checkboxVal = localStorage.getItem('annealHeaders') === null ? this.formTheadOptions : localStorage.getItem('annealHeaders').split(',')
      this.formThead = arr
    ...
    handleDeliverSubmit() {
      ...
      if (this.motionRadio === 0) {
        ...
        for (const item of this.autoParams) {
          if (item.val === '') {
            this.$message({
              type: 'error',
              message: '请输入参数信息!'
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 15 }).then(res => {
