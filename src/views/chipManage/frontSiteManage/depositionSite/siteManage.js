...
import { siteSelect, acceptSliceScan, acceptSlice, cutSlice, upSlice, sendSlice, getRemark, getProgram, findMachinList, downLoadFile, findSiteException, findUserByRoleName } from '@/api/chipManage/frontSiteManage/cleanSite'
...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('deposionHeaders') === null ? this.formTheadOptions : localStorage.getItem('deposionHeaders').split(',')
  ...
  methods: {
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=14&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    getParamsData(row) {
      ...
      console.log(this.handTestParams)
    },
    // 获取机台
    findMachinListFun() {
      const params = {
    ...
    submitOption() {
      ...
      localStorage.setItem('deposionHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=14&siteName=沉积站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    handleTransmitDialog() {
      ...
      for (const item of this.handTestParams) {
        ...
        if (JSON.stringify(this.exception) !== '{}') {
          if (item.paramsCode === 'mohou') {
            if (parseInt(item.val) < parseInt(this.exception.mohou.split(',')[0]) || parseInt(item.val) > parseInt(this.exception.mohou.split(',')[1])) {
              this.$message({
                type: 'error',
                message: '膜厚参数信息异常，请先进行异常上报!'
              })
              flags = false
              break
            }
          }
          if (item.paramsCode === 'zheshelv') {
            if (parseInt(item.val) < parseInt(this.exception.zheshelv.split(',')[0]) || parseInt(item.val) > parseInt(this.exception.zheshelv.split(',')[1])) {
              this.$message({
                type: 'error',
                message: '折射率参数信息异常，请先进行异常上报!'
              })
              flags = false
              break
            }
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 14 }).then(res => {
