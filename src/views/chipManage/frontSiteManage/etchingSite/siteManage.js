...
import { siteSelect, acceptSliceScan, acceptSlice, cutSlice, upSlice, sendSlice, getRemark, getProgram, findMachinList, getRunCodeList, downLoadFile, findSiteException, findUserByRoleName } from '@/api/chipManage/frontSiteManage/cleanSite'
...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('etchingSiteHeaders') === null ? this.formTheadOptions : localStorage.getItem('etchingSiteHeaders').split(',')
    this.setThead(this.checkboxVal)
  },
  methods: {
    getRunCodeListFun() {
      getRunCodeList({ siteId: 12, processId: this.jProcess }).then(res => {
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=12&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    machineChange(val) {
      this.machinList.map(item => {
        if (item.id === val) {
          this.slotList = item.slotNo.split(',')
    ...
    submitOption() {
      ...
      localStorage.setItem('etchingSiteHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=12&siteName=蚀刻站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    handleTransmitDialog() {
      ...
      for (const item of this.handTestParams) {
        if (item.val === '' && item.paramsCode !== 'runbianhao') {
          this.$message({
            type: 'error',
            message: '请输入参数信息!'
          })
        ...
        if (JSON.stringify(this.exception) !== '{}') {
          if (item.paramsCode === 'shike13') {
            if (parseInt(item.val) < parseInt(this.exception.shike13.split(',')[0]) || parseInt(item.val) > parseInt(this.exception.shike13.split(',')[1])) {
              this.$message({
                type: 'error',
                message: '蚀刻深度（13#）参数信息异常，请先进行异常上报!'
              })
              return false
            }
          }
          if (item.paramsCode === 'shike20') {
            if (parseInt(item.val) < parseInt(this.exception.shike20.split(',')[0]) || parseInt(item.val) > parseInt(this.exception.shike20.split(',')[1])) {
              this.$message({
                type: 'error',
                message: '蚀刻深度（20#）参数信息异常，请先进行异常上报!'
              })
              return false
            }
          }
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 12 }).then(res => {
