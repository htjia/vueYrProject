  ...
  <PageHeaderLayout >
    ...
    <el-dialog
      ...
      <div class="overflow-hidden input-container">
        ...
        <div class="input-item" style="margin-top: 0;">
          ...
          <el-input :disabled="true" value="蒸镀" class="search-input-scan" size="small" maxlength="20" clearable/>
        </div>
      ...
      <el-row :gutter="15">
        <el-col :span="8">
          ...
          <div class="left-content" style="height: auto">
            <span class="input-title" style="margin-top: 15px; width: 70px">机台编号 </span>
            <el-select :disabled="jProcess === ''" v-model="machineNum" class="dialog-input-short" size="small" style="width: 245px" placeholder="请选择" filterable clearable @change="machineChange">
              <el-option
            ...
            <span v-if="machineType === 0" class="input-title" style="margin-top: 15px; width: 70px">槽位 </span>
            <el-select v-if="machineType === 0" :disabled="machineNum === ''" v-model="trench" class="dialog-input-short" style="width: 245px;" size="small" placeholder="请选择" filterable clearable>
              <el-option
            ...
            <span v-if="machineType === 1" class="input-title" style="margin-top: 15px; width: 70px">胶管 </span>
            <el-select v-if="machineType === 1" :disabled="machineNum === ''" v-model="trench" class="dialog-input-short" style="width: 245px;" size="small" placeholder="请选择" filterable clearable>
              <el-option
            ...
            <!--<span class="input-title" style=" width: 150px;">累计Run次（原料1） </span>
            <el-input v-model="runNum1" class="dialog-input-short margin-top" style="width: 165px;" size="small" placeholder="累计Run次（原料1）" type="number" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}" clearable/>
            <span class="input-title" style=" width: 150px;">阈值（原料1） </span>
            <el-input v-model="threshold1" class="dialog-input-short margin-top" style="width: 165px;" size="small" placeholder="请输入阈值（原料1）" type="number" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}" clearable/>
            <span class="input-title" style=" width: 150px;">累计Run次（原料2） </span>
            <el-input v-model="runNum2" class="dialog-input-short margin-top" style="width: 165px;" size="small" placeholder="累计Run次（原料1）" type="number" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}" clearable/>
            <span class="input-title" style=" width: 150px;">阈值（原料2） </span>
            <el-input v-model="threshold2" class="dialog-input-short margin-top" style="width: 165px;" size="small" placeholder="请输入阈值（原料1）" type="number" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}" clearable/>-->
            <span class="input-title" style=" width: 70px;">操作员 </span>
            <el-select v-model="jOperator" class="dialog-input-short margin-top" style="width: 245px" size="small" placeholder="请选择" filterable clearable>
              <el-option
                v-for="item in userOptions"
                :key="item.id"
                :label="item.name"
            ...
            <el-input v-model="jRemarkLeft" class="dialog-input-short" style="margin-top: 10px;width: 245px" size="small" type="textarea" placeholder="请输入备注" maxlength="50" clearable/>
          </div>
          <!--参数数据-->
          <span v-if="autoParams.length > 0 || handParams.length > 0" class="module-title-text" style="float:none; margin-top: 15px;margin-bottom: 0px;display: inline-block">参数数据</span>
          <div v-if="autoParams.length > 0 || handParams.length > 0" class="left-content" style="height: auto">
            <div>
              <span v-if="electrodeMaterials.length > 0" class="input-title" style="margin-top: 15px; width: 70px">电极材料</span>
              <span v-for="(material, index) in electrodeMaterials" :key="material.id">{{ material.name }} <span v-if="index !== electrodeMaterials.length - 1">,</span></span>
            </div>
            <!--<div v-for="item in handParams" :key="item.paramsName">
              <span class="input-title" style="margin-top: 15px; width: 120px">{{ item.paramsName }}</span>
              <el-input v-model="item.val" class="dialog-input-short" style="margin-top: 10px;width: 195px" size="small" maxlength="20"/>
            </div>
            <div v-for="item in autoParams" :key="item.paramsName">
              <span class="input-title" style="margin-top: 15px; width: 120px">{{ item.paramsName }}</span>
              <el-input v-model="item.val" class="dialog-input-short" style="margin-top: 10px;width: 195px" size="small" maxlength="20" clearable/>
            </div>-->
            <div>
              <span class="input-title" style="margin-top: 15px; width: 70px">Run编号</span>
              <el-input v-model="RunCode" class="dialog-input-short" style="margin-top: 10px;width: 245px" size="small" maxlength="20" clearable/>
            </div>
            <!--电极-->
            <el-table
              v-if="handParams.length > 0"
              :data="circleList"
              :span-method="mSpanMethod"
              border
              class="mo-table"
              max-height="180"
              style="width: 100%; margin-top: 5px">
              <el-table-column width="70" label="蒸镀材料" prop="materialName" align="center" />
              <el-table-column label="批次号" prop="materialBatch" align="center" />
              <el-table-column label="蒸镀前重量" prop="materialModel" align="center">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.befoeWeight" size="mini" type="number" style="width: 100%"/>
                </template>
              </el-table-column>
              <el-table-column label="操作" width="70" align="center" >
                <template slot-scope="scope">
                  <el-button v-if="scope.row.materialName" class="view-all" style="float: none" type="primary" size="mini" @click="moReplace(scope.row)"><svg-icon icon-class="change"/> 更换</el-button>
                </template>
              </el-table-column>
            </el-table>
            <!--ITO-->
            <el-table
              v-if="autoParams.length > 0"
              :data="circleList"
              :span-method="mSpanMethod"
              border
              class="mo-table"
    ...
    <el-dialog
      ...
      <el-row :gutter="15">
        <el-col :span="8">
          ...
          <!--参数数据-->
          <span v-if="handTestParams.length > 0 || autoTestParams.length > 0" class="module-title-text" style="float:none; margin-top: 15px;margin-bottom: 0px;display: inline-block">传片信息</span>
          <!--<el-radio-group v-model="motionRadio" class="float-right" style="margin-top: 15px"><el-radio :label="0">电极蒸镀</el-radio><el-radio :label="1">ITO蒸镀</el-radio></el-radio-group>-->
          <div v-if="handTestParams.length > 0 || autoTestParams.length > 0" class="left-content" style="height: auto">
            <span v-if="setShow('runbianhao')" class="input-title" style="margin-top: 15px; width: 70px">Run编号 </span>
            <el-select v-if="setShow('runbianhao')" v-model="runCode" class="dialog-input-short" style="margin-top: 10px;width: 245px" size="small" placeholder="请选择" filterable @change="runCodeChange" >
              <el-option
                v-for="item in runCodeList"
                :key="item"
                :label="item"
                :value="item"/>
            </el-select>
            <span v-if="setShow('zhengduhoudu')" class="input-title" style="margin-top: 15px; width: 70px">蒸镀厚度</span>
            <el-input v-if="setShow('zhengduhoudu')" v-model="evaporationThickness" class="dialog-input-short" style="margin-top: 10px;width: 245px" size="small" type="number" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}" @input="evaporationThicknessChange"/>
            <span v-if="setShow('qianzhongliang')" class="input-title" style="margin-top: 15px; width: 120px">蒸镀前重量(g)</span>
            <el-input v-if="setShow('qianzhongliang')" v-model="beforeWeight" :disabled="true" class="dialog-input-short" style="margin-top: 10px;width: 195px" size="small" maxlength="20" clearable/>
            <span v-if="setShow('houzhongliang')" class="input-title" style="margin-top: 15px; width: 120px">蒸镀后重量(g)</span>
            <el-input v-if="setShow('houzhongliang')" v-model="afterWeight" class="dialog-input-short" style="margin-top: 10px;width: 195px" type="number" size="small" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}" @input="afterWeightChange"/>
            <span v-if="setShow('jingzhongliang')" class="input-title" style="margin-top: 15px; width: 120px">净重量(g)</span>
            <el-input v-if="setShow('jingzhongliang')" :disabled="true" v-model="netWeight" class="dialog-input-short" style="margin-top: 10px;width: 195px" size="small" maxlength="20"/>
            <span v-if="setAutoTestParamsShow('piandianzu')" class="input-title" style="margin-top: 15px; width: 120px">Run编号 </span>
            <el-select v-if="setAutoTestParamsShow('piandianzu')" v-model="runCode" class="dialog-input-short" style="margin-top: 10px;width: 195px" size="small" placeholder="请选择" filterable @change="runCodeChange" >
              <el-option
                v-for="item in runCodeList"
                :key="item"
                :label="item"
            ...
            <span v-if="setAutoTestParamsShow('piandianzu')" class="input-title" style="margin-top: 15px; width: 120px">片电阻</span>
            <el-input v-if="setAutoTestParamsShow('piandianzu')" v-model="piandianzu" class="dialog-input-short" style="margin-top: 10px;width: 195px" type="number" size="small" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}"/>
            <span v-if="setAutoTestParamsShow('fengzhibochang')" class="input-title" style="margin-top: 15px; width: 120px">峰值波长</span>
            <el-input v-if="setAutoTestParamsShow('fengzhibochang')" v-model="fengzhibochang" class="dialog-input-short" style="margin-top: 10px;width: 195px" type="number" size="small" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}"/>
            <span v-if="setAutoTestParamsShow('fengzhitouguangdu')" class="input-title" style="margin-top: 15px; width: 120px">峰值透光度</span>
            <el-input v-if="setAutoTestParamsShow('fengzhitouguangdu')" v-model="fengzhitouguangdu" class="dialog-input-short" style="margin-top: 10px;width: 195px" type="number" size="small" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}"/>
            <span v-if="setAutoTestParamsShow('touguangdu')" class="input-title" style="margin-top: 15px; width: 120px">460nm透光度</span>
            <el-input v-if="setAutoTestParamsShow('touguangdu')" v-model="touguangdu" class="dialog-input-short" style="margin-top: 10px;width: 195px" type="number" size="small" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}"/>
            <!--<div v-for="item in autoTestParams" :key="item.paramsName">
              <span class="input-title" style="margin-top: 15px; width: 120px">{{ item.paramsName }}</span>
              <el-input v-model="item.vals" class="dialog-input-short" style="margin-top: 10px;width: 195px" size="small" maxlength="20" type="number" onkeyup="if(this.value.length > 10){this.value = this.value.slice(0, 10)}" clearable/>
            </div>-->
            <el-table
              v-if="handTestParams.length === 2"
              :data="materialList"
              :span-method="mSpanMethod"
              border
              class="mo-table"
              ...
              style="width: 100%; margin-top: 5px">
              <el-table-column width="70" label="蒸镀材料" prop="materialName" align="center"/>
              <el-table-column label="蒸镀前重量" align="center">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.befoeWeight" size="mini" type="number" style="width: 100%" disabled/>
                </template>
              </el-table-column>
              <el-table-column label="蒸镀后重量" align="center">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.afterWeight" size="mini" type="number" style="width: 100%" @blur="afterWeightBlur(scope.row)"/>
                </template>
              </el-table-column>
              <el-table-column label="净重量" align="center">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.weight" size="mini" type="number" style="width: 100%" disabled/>
                </template>
              </el-table-column>
            </el-table>
          </div>
    ...
    <el-dialog
      ...
      <el-form ref="mocvdForm" :validate-on-rule-change="false" :rules="rules" :model="mocvdForm" status-icon label-width="120px" label-position="right">
        <div class="module-title" style="margin-top: -10px;margin-bottom: 10px;height: 35px;">
          <div class="module-title-text">更换后原材料信息</div>
        </div>
        <el-row :gutter="20">
          <el-col :span="12">
      ...
      <el-form ref="disForm" :model="disForm" status-icon label-width="120px" label-position="right">
        <div class="module-title" style="margin-top: -10px;margin-bottom: 10px;height: 35px;">
          <div class="module-title-text">更换前原材料信息</div>
        </div>
        <el-row :gutter="20">
          <el-col :span="12">
        ...
        <el-form-item label="备注 " size="small">
          <el-input v-model="disForm.remark" :disabled="true" type="textarea" maxlength="50"/>
        </el-form-item>
      </el-form>
      ...
      <el-table
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-show="runStatus === 3" type="primary" size="mini" @click="handleJiePian(scope.row, 3)"><svg-icon icon-class="xiangyou"/>传片</el-button>
          </template>
        </el-table-column>
      </el-table>
      <span slot="footer" class="dialog-footer">
