...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('developHeaders') === null ? this.formTheadOptions : localStorage.getItem('developHeaders').split(',')
  ...
  methods: {
    ...
    inputChange(data) {
      if (data.paramsCode === 'xainyingshijian' && parseInt(data.val) < 0) {
        data.val = 0
      }
    },
    getParamsData(row) {
    ...
    submitOption() {
      ...
      localStorage.setItem('developHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=9&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    // 扫描回车
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=9&siteName=显影站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 9 }).then(res => {
