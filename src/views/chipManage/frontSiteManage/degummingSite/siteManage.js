...
import { siteSelect, acceptSliceScan, acceptSlice, cutSlice, upSlice, sendSlice, getRemark, getProgram, findMachinList, downLoadFile, findUserByRoleName, findGrandTotal } from '@/api/chipManage/frontSiteManage/cleanSite'
...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('degummingHeaders') === null ? this.formTheadOptions : localStorage.getItem('degummingHeaders').split(',')
  ...
  methods: {
    ...
    inputChange(data) {
      if (data.paramsCode === 'fushishijian' && parseInt(data.val) < 0) {
        data.val = 0
      }
    },
    getParamsData(row) {
    ...
    findGrandTotalFun(machineId) {
      const requestParams = {
        machineId: this.machineNum,
        localtion: this.trench
      }
      findGrandTotal(requestParams).then(res => {
        this.grandTotal = res.data.num
        this.threshold = res.data.yuval
      })
    },
    deleteItem(index) {
    ...
    submitOption() {
      ...
      localStorage.setItem('degummingHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    labelHead(h, { column, index }) {
      ...
      column.minWidth = 160 // 字大小乘个数即长度 ,注意不要加px像素，这里minWidth只是一个比例值，不是真正的长度
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=4&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    // 扫描回车
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=4&siteName=去胶站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    handleDeliverSubmit() {
      ...
      paramsList.leijipianshu = this.grandTotal
      paramsList.yuzhi = this.threshold
      const params = {
        siteId: 4,
        processId: this.jProcess,
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 4 }).then(res => {
        this.proceOptions = res.data
      })
    },
    // 人员
    findMachineUserFun() {
      findUserByRoleName({ roleName: '清洗车间' }).then(res => {
