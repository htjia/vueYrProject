...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('fuseHeaders') === null ? this.formTheadOptions : localStorage.getItem('fuseHeaders').split(',')
  ...
  methods: {
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=13&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    submitOption() {
      ...
      localStorage.setItem('fuseHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=13&siteName=熔合站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 13 }).then(res => {
