...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('clearHeaders') === null ? this.formTheadOptions : localStorage.getItem('clearHeaders').split(',')
  ...
  methods: {
    ...
    submitOption() {
      ...
      localStorage.setItem('clearHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=1&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    // 扫描回车
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=1&siteName=清洗站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 1 }).then(res => {
