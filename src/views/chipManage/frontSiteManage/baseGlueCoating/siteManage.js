...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('baseGlueHeaders') === null ? this.formTheadOptions : localStorage.getItem('baseGlueHeaders').split(',')
  ...
  methods: {
    ...
    submitOption() {
      ...
      localStorage.setItem('baseGlueHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=8&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    // 扫描回车
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=8&siteName=底胶涂布站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 8 }).then(res => {
