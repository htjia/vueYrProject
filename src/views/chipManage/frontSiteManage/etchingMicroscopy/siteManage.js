...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('etchingHeaders') === null ? this.formTheadOptions : localStorage.getItem('etchingHeaders').split(',')
  ...
  methods: {
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=18&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    submitOption() {
      ...
      localStorage.setItem('etchingHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=18&siteName=腐蚀镜检&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 18 }).then(res => {
