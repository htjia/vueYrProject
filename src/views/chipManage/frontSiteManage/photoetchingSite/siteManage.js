...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('photoetchingHeaders') === null ? this.formTheadOptions : localStorage.getItem('photoetchingHeaders').split(',')
  ...
  methods: {
    ...
    submitOption() {
      ...
      localStorage.setItem('photoetchingHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=5&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    // 扫描回车
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=5&siteName=光刻站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    handleDeliverSubmit() {
      ...
      const paramsList = {}
      paramsList.yuancailiao = this.material
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 5 }).then(res => {
