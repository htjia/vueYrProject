...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('hardBakeHeaders') === null ? this.formTheadOptions : localStorage.getItem('hardBakeHeaders').split(',')
  ...
  methods: {
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=6&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    inputChange(data) {
      if (data.paramsCode === 'yingkaoshijian' && parseInt(data.val) < 0) {
        data.val = 0
      }
    },
    getParamsData(row) {
    ...
    submitOption() {
      ...
      localStorage.setItem('hardBakeHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=6&siteName=硬烤站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 6 }).then(res => {
