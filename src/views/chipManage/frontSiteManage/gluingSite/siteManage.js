...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('gluingHeaders') === null ? this.formTheadOptions : localStorage.getItem('gluingHeaders').split(',')
  ...
  methods: {
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=3&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    submitOption() {
      ...
      localStorage.setItem('gluingHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=3&siteName=打胶站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 3 }).then(res => {
