...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('beforLeaveHeaders') === null ? this.formTheadOptions : localStorage.getItem('beforLeaveHeaders').split(',')
  ...
  methods: {
    ...
    downLoadFile(batchNo, processId) {
      ...
      downLoadFile(params).then(res => {
        window.open(process.env.BASE_API + `/xp-produceTrack/down?siteId=19&batchNo=${batchNo}&processId=${processId}`)
      })
    },
    getListFun() {
      const params = {
    ...
    submitOption() {
      ...
      localStorage.setItem('beforLeaveHeaders', this.checkboxVal)
      this.formThead = arr
    ...
    exportExcel() {
      ...
      window.open(process.env.BASE_API + `/xp-beforeProcessManage/export?siteId=19&siteName=出站镜检站点&batchNo=${this.batchNum}&processId=${this.process}&startTime=${startTime}&endTime=${endTime}&timeType=${this.timeRadio}`)
    },
    // 接片
    handleReceive() {
    ...
    handleDeliverSubmit() {
      ...
      const paramsList = { machineCode: this.machineCode }
      const params = {
        siteId: 19,
        processId: this.jProcess,
    ...
    // 获取对应站点的工序
    nowProcessListFun() {
      nowProcessList({ siteId: 19 }).then(res => {
        this.proceOptions = res.data
      })
    },
    // 人员
    findMachineUserFun() {
      findUserByRoleName({ roleName: '镜检车间' }).then(res => {
