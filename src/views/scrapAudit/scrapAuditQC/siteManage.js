...
import { queryHasScrapped, cancelPendingData, generateBillNo, addBill, getHasScrapDetail, deleteBill, updateBill, review } from '@/api/scrapManage/scrap'
export default {
  ...
  methods: {
    ...
    submitBill() {
      if (this.dialogType === 1 && (this.rowInfo.status === 0 || this.rowInfo === null)) {
        if (this.selectList.length === 0) {
          this.$message({
            type: 'error',
            message: '请选择需要报废的批号!'
    ...
    importDBF() {
      ...
      window.open(process.env.BASE_API + `/xpProductExceptionScrapController/exportData?batchNo=${this.batchNo}&startTime=${startTime}&endTime=${endTime}&typeId=${this.tpType}&sizeId=${this.measureId}&productModel=${this.productId}&waferNo=${this.waferId}`)
    },
    findInfo(row) {
      this.rowInfo = row
    ...
    getHasScrapDetail(row) {
      ...
      this.dialogForm = {
        ...
        qcuserId: this.dialogType === 3 && !this.checkDialogVisible ? parseInt(sessionStorage.getItem('User-Id')) : row.auditor,
        qccreatTime: this.dialogType === 3 && !this.checkDialogVisible ? new Date().getTime() : new Date(row.auditTime).getTime(),
