...
import { getList, getInfo, getInfoFromBatch, productData, pause } from '@/api/chipManage/progressTracking'
...
export default {
  ...
  mounted() {
    ...
    this.checkboxVal = localStorage.getItem('progressHeaders').split(',')
  ...
  methods: {
    ...
    viewProgress(row) {
      ...
      getInfoFromBatch({ batchNoList: row.batchNo }).then(res => {
        this.sliceList = res.data.batchInfo[0].sliceInfo
        this.sliceListStart = this.sliceList.slice(0, 12)
        this.sliceListEnd = this.sliceList.slice(12)
      })
