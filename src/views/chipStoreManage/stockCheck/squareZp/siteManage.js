...
import { findFpQuery, updateFpCheck, wavelength, brightness, voltage, concentration, comprehensive } from '@/api/chipStoreManage/stockCheck'
...
export default {
  ...
  methods: {
    ...
    // 待查
    handleInvestigated() {
      if (this.selectedRunRow.status === '在库' || this.selectedRunRow.status === '待查') {
        const message = this.selectedRunRow.status === '在库' ? '是否确认修改当前wafer状态为待查？' : '是否确认恢复当前wafer状态为在库？'
        this.$confirm(message, '提示', {
          ...
          const params = {
            ...
            status: this.selectedRunRow.status === '在库' ? 5 : 0
          }
          updateFpCheck(params).then(res => {
            this.$message({
              type: 'success',
              message: '操作成功!'
            })
