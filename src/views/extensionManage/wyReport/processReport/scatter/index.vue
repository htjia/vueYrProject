  ...
  methods: {
    initChart() {
      ...
      this.datas.series.map((item, index) => {
        ...
        this.options.yAxis[index].min = min
        this.options.yAxis[index].max = max
        legendData.push(item.name)
        item = Object.assign(item, obj)
      })
      if (this.datas.series.length > 1) {
        this.datas.series[1]['yAxisIndex'] = 1
      }
      ...
      this.chart.on('click', function(e) {
        console.log(e)
        console.log(_this.options.xAxis.data)
        console.log(_this.options.xAxis.data[e.dataIndex])
        let data = []
